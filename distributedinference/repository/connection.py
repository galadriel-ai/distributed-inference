...
class SessionProvider:
    ...
    def start_connection(cls, logger: logging.Logger, user: str, password: str, db: str, host: str, port: int,
                         ...
                         pool_timeout: int = DEFAULT_POOL_TIMEOUT) -> 'SessionProvider':
...
db_session_provider = {"write": SessionProvider.start_connection(logger=api_logger.get(),
                                                                 user=settings.DB_USER,
                                                                 password=settings.DB_PASSWORD,
                                                                 db=settings.DB_DATABASE,
                                                                 host=settings.DB_HOST,
                                                                 port=settings.DB_PORT),
                       "read": SessionProvider.start_connection(logger=api_logger.get(),
