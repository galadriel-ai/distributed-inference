...
class GoogleCloudStorage:
    ...
    async def decode_b64_and_upload_to_gcs(self, request_id: str, idx: int, image_b64: str) -> str:
        if not self.client:
        ...
        return await loop.run_in_executor(None, self._upload_to_gcs, blob_name, image_bytes)
    ...
    def _upload_to_gcs(self, blob_name: str, image_bytes: bytes) -> str:
        ...
        url = blob.generate_signed_url(
            version="v4",
            expiration=datetime.timedelta(minutes=URL_EXPIRATION_MINUTES),
            method="GET",
        )
        return url
