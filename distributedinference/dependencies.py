...
"""
This module contains the dependencies used by the FastAPI application.
The objects in this module are singletons and are reused across the application.
"""
...
_node_repository_instance = NodeRepository(session_provider=db_connection.get(),
                                           session_provider_read=db_connection_read.get())
_user_node_repository_instance = UserNodeRepository(session_provider=db_connection.get(),
                                                    session_provider_read=db_connection_read.get())
_user_repository_instance = UserRepository(session_provider=db_connection.get(),
                                           session_provider_read=db_connection_read.get())
_node_stats_repository_instance = NodeStatsRepository(session_provider=db_connection.get(),
                                                      session_provider_read=db_connection_read.get())
_benchmark_repository_instance = BenchmarkRepository(session_provider=db_connection.get(),
                                                     session_provider_read=db_connection_read.get())
_metrics_queue_repository = MetricsQueueRepository()
_metrics_repository = MetricsRepository(session_provider=db_connection.get(),
                                        session_provider_read=db_connection_read.get())
_rate_limit_repository = RateLimitRepository(session_provider=db_connection.get(),
                                             session_provider_read=db_connection_read.get())
_billing_repository = BillingRepository(session_provider=db_connection.get(),
                                        session_provider_read=db_connection_read.get())
_tokens_queue_repository = TokensQueueRepository()
_tokens_repository_instance = TokensRepository(session_provider=db_connection.get(),
                                               session_provider_read=db_connection_read.get())
...
def get_tokens_repository() -> TokensRepository:
    return _tokens_repository_instance
