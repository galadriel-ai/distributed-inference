...
async def execute(
    ...
    connected_node_metrics = await repository.get_connected_node_metrics(node_info.node_id)
    return GetNodeInfoResponse(
        ...
        node_created_at=0 if not node_info.created_at else int(node_info.created_at.timestamp()),
    )
